...
def canon_with_l_param(step, k, handler):
    ...
    x_lower = handler.var_lowerbounds[lrange1D_handler.index_matrix()]
    x_upper = handler.var_upperbounds[lrange1D_handler.index_matrix()]
    for i in range(n):
        ...
        # if not handler.add_planet:
        #     l_li = l_lower[i, 0]
        #     x_li = x_lower[i, 0]
        #     x_ui = x_upper[i, 0]
        #     if x_li <= l_li and x_ui <= l_li:
        #         b_uvals.append(0)
        #     else:
        #         b_uvals.append(np.inf)
        # else:
