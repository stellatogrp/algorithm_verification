...
def plot_resids(df_srlt, df_g):
    ...
    for i, eps in enumerate(eps_vals):
        tempdf_srlt = df_srlt[df_srlt['eps_b'] == eps]  # df[df['num_iter'] == N]
        tempdf_g = df_g[df_g['eps_b'] == eps]
        ...
        ax.plot(N_vals, srlt_obj, label=f'eps={eps}', color=colors[i])
...
def main():
    ...
    s_rlt_fname = data_dir + 'sdp_rlt.csv'
    g_fname = data_dir + 'global.csv'
    # plot_times(df)
